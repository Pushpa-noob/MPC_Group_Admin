...
@Component({
  selector: 'app-user-wise-pl',
  templateUrl: './user-wise-pl.component.html',
  styleUrls: ['./user-wise-pl.component.scss']
})
export class UserWisePlComponent {
  ...
  downloadCSV() {
    this.exportService.exportToCSV(this.marketPL, 'UserWisePL');
}

downloadExcel() {
    this.exportService.exportToExcel(this.marketPL, 'UserWisePL');
}

downloadPDF() {
    this.exportService.exportToPDF(this.marketPL, 'UserWisePL');
}
  ...
  getUserWisePL() {
    this.marketPL = [];
    this.spinner.show();
    this.accountService.getUserWisePL(this.userId, this.eventId,this.marketName, this.startTime, this.endTime, this.skipRecords, this.takeRecords, this.parentId).subscribe({
      next: (response: any) => {
  ...
  getEventWiseBetHistory(userId:number) {
    this.betHis = []
    this.spinner.show();
    this.accountService.getEventWiseBetHistory("", userId,this.marketName, this.startTime, this.endTime, this.skipRecords, this.takeRecords).subscribe({
      next: (response: any) => {
        if (response.status) {
